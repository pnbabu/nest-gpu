...
syn_group = ngpu.CreateSynGroup \
            ("stdp_synapse", {"tau_plus":tau_plus, "tau_minus":tau_minus, \
...
ngpu.ActivateRecSpikeTimes(neuron0, 10)
neuron1 = ngpu.Create("aeif_cond_beta", N)
ngpu.ActivateRecSpikeTimes(neuron1, 10)
...
actual_pre_spike_times = ngpu.GetRecSpikeTimes(neuron0)
actual_post_spike_times = ngpu.GetRecSpikeTimes(neuron1)

# print(actual_pre_spike_times, actual_post_spike_times)
...
plt.figure(1)
plt.plot(dt, sim_w, label="simulated")
plt.plot(dt, expect_w, ls="--", label="expected")
